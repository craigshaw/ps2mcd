...
class PS2MC():
    ...
    def _validate(self):
        ...
        if self.page_size not in VALID_PAGE_SIZES or \
           self.page_size == 512 and not self.pages_per_cluster in (1,2) or \
    ...
    def _unpack_directory_entries(self, dbuffer, path, length):
        ...
        for i in range(length):
            d = self._unpack_directory_entry(dbuffer[(i*DIRECTORY_SIZE):(i*DIRECTORY_SIZE)+DIRECTORY_SIZE])

            if d[7].decode('UTF-8').rstrip('\x00') != '':
                dirs.append(DirectoryEntry(d, path))
    ...
    def _read_allocatable_cluster(self, offset):
        o1 = (self.alloc_offset * (self.cs + (self.ecc_len*self.pages_per_cluster))) + \
        (offset * (self.cs + (self.ecc_len*self.pages_per_cluster)))
    ...
    def _read_cluster_from(self, offset):
        ...
        for _ in range(self.pages_per_cluster):
    ...
    def write_all_to_disk(self, directory):
        """
        Write all files on the memory card to disk at the specified directory.
        :param directory: The directory to dump the files to.
        """
        for f in self.files:
            contents = self._read_file(f)

            path = directory / f.to_path()
